...
![Ingestion Architecture](./ingestion-flow-chart.png)
...
Ingestion checks required local tools:
...
It does not currently download source from `sourceUrl`; it throws if the archive is missing.
...
```txt
.ingest/extract/<arxivId>/
```

Then extracted into:

```txt
.ingest/extract/<arxivId>/src
```
...
arXiv source trees dont have a structured format so this is the best that I could come up with
...
This is a lossy but practical step. The goal is not perfect document reproduction; it is better retrieval-quality text extraction
...
```txt
.ingest/extract/<arxivId>/sections
```
...
This is better than dumping all sections from all papers into one global vector space because paper titles and author metadata help differentiate the target paper before section search
